...
function exercicio02() {
    ...
    const mensagem = idade >= 18
        ? `Você tem ${idade} anos. Já pode tirar CNH.`
        : `Você tem ${idade} anos. Ainda não pode tirar CNH.`;
...
function exercicio05() {
    ...
    mostrarResultado(ehBissexto ? `O ano ${ano} é bissexto.` : `O ano ${ano} não é bissexto.`);
}
...
function exercicio07() {
    ...
    if (!formaTriangulo) return mostrarResultado("Esses valores não formam triângulo.");
    if (ladoA === ladoB && ladoB === ladoC) return mostrarResultado("Forma triângulo Equilátero.");
    if (ladoA === ladoB || ladoA === ladoC || ladoB === ladoC) return mostrarResultado("Forma triângulo Isósceles.");
    return mostrarResultado("Forma triângulo Escaleno.");
}
